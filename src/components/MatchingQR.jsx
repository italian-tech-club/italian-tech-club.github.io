...
const MatchingQR = () => {
  const matchingLink = "https://italiantechclubnyc.com/cofounder-matching";
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Find Your Co-Founder — Italian Tech Club NYC',
          text: 'Looking for a co-founder? Join the Italian Tech Club co-founder matching!',
          url: matchingLink,
        });
      } catch (err) {
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(matchingLink);
    setCopied(true);
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50 border border-slate-100 dark:border-slate-800 transform hover:scale-[1.01] transition-transform duration-300 w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Scan to join the Italian Tech Club co-founder matching
            </p>
          </div>
